...
def _unit_filters(status: str, source: str | None) -> tuple[str, list[Any]]:
    ...
    if status == "reviewed":
        ...
        clauses.append("r.reviewed = 0 AND json_array_length(r.fra) != json_array_length(r.mos)")
    elif status != "all":
    ...
    *,
    ...
    with connect(path) as db:
        rows = db.execute(
            ...
            r.version, r.reviewed, r.reviewed_by
            FROM units u JOIN reviews r ON r.unit_id = u.id
            {where}
            ORDER BY u.source, u.position, u.id{pagination}""",
            params,
        ).fetchall()
    return [{**dict(row), "fra": json.loads(row["fra"]), "mos": json.loads(row["mos"])} for row in rows]
...
def review_summary(path: Path) -> dict[str, int]:
    with connect(path) as db:
        row = db.execute(
            ...
            COALESCE(SUM(
                r.reviewed = 0 AND json_array_length(r.fra) != json_array_length(r.mos)
            ), 0) AS mismatched
            FROM reviews r"""
        ).fetchone()
...
def get_unit(path: Path, unit_id: int) -> dict[str, Any]:
    ...
    return {
        **dict(row),
        "fra": json.loads(row["fra"]),
        "mos": json.loads(row["mos"]),
        "original_fra": json.loads(row["original_fra"]),
        "original_mos": json.loads(row["original_mos"]),
    }
...
def get_draft(path: Path, unit_id: int, reviewer: str) -> dict[str, Any] | None:
    with connect(path) as db:
        row = db.execute(
            "SELECT fra_text, mos_text, base_version FROM drafts WHERE unit_id = ? AND reviewer = ?",
            (unit_id, reviewer),
        ).fetchone()
...
def save_draft(
    path: Path, unit_id: int, reviewer: str, fra_text: str, mos_text: str, base_version: int
) -> None:
    with connect(path) as db:
        db.execute(
            """INSERT INTO drafts (unit_id, reviewer, fra_text, mos_text, base_version)
            VALUES (?, ?, ?, ?, ?)
            ...
            (unit_id, reviewer, fra_text, mos_text, base_version),
        )
...
def accept_review(
    path: Path, unit_id: int, reviewer: str, fra_text: str, mos_text: str, expected_version: int
) -> int:
    fra, mos = split_lines(fra_text), split_lines(mos_text)
    if not fra or len(fra) != len(mos):
        raise ValueError(
            f"French and Mooré must have the same nonzero number of lines ({len(fra)} / {len(mos)})."
        )
    with connect(path) as db:
        cursor = db.execute(
            """UPDATE reviews SET fra = ?, mos = ?, version = version + 1,
            reviewed = 1, reviewed_by = ?, updated_at = CURRENT_TIMESTAMP
...
def iter_pairs(path: Path, reviewed_only: bool = False) -> Iterator[dict[str, str]]:
    ...
    with connect(path) as db:
        rows = db.execute(
            f"""SELECT u.source, u.unit_uid, r.fra, r.mos
            FROM units u JOIN reviews r ON r.unit_id = u.id{where}
            ORDER BY u.source, u.position, u.id"""
        )
        for row in rows:
            fra_sentences, mos_sentences = json.loads(row["fra"]), json.loads(row["mos"])
